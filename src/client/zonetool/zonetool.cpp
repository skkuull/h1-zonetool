...
#define DUMP_ASSET(__type__,__interface__,__struct__) \
	...
	void reallocate_asset_pool(const XAssetType type, const unsigned int new_size)
	{
		...
		std::memmove(new_pool, DB_XAssetPool[type], g_poolSize[type] * element_size);

		DB_XAssetPool[type] = new_pool;
	...
	void dump_zone(const std::string& name)
	{
		...
		while (!WaitForSingleObject(*reinterpret_cast<HANDLE*>(0xAC80958_b), 0) == 0)
		{
			Sleep(1);
		}
	...
	void verify_zone(const std::string& name)
	{
		...
		// wait for database to be ready
		while (!WaitForSingleObject(*reinterpret_cast<HANDLE*>(0xAC80958), 0) == 0)
		{
			Sleep(1);
		}

		verify = true;
		load_zone(name, DB_LOAD_ASYNC, true);
	}
	...
	void init_zonetool()
	{
		...
		// enable dumping
		DB_LinkXAssetEntry1_hook.create(0x396E80_b, &DB_LinkXAssetEntry1);

		// stop dumping
		DB_FinishLoadXFile_hook.create(0x367FF0_b, &DB_FinishLoadXFile);

		doexit_hook.create(0x83D0D4_b, doexit);
		atexit(on_exit);
	}
